...
const CreatAccountAdmin: React.FC = () => {
  ...
  const validateExcelFile = (data: any[]) => {
    ...
    // 🟢 Optional fields like "Middle Initial" and "Suffix" are not required anymore
  };
  const handleBulkRegister = async () => {
    ...
    try {
      for (const u of pendingUsers) {
        ...
        // Get the role name from rolesList (match by ID or name)
        const matchedRole =
          rolesList.find(
            (r) =>
        ...
        await set(ref(db, `users/${uid}`), {
          ...
          suffix,
          email,
          role: matchedRole, // ← use the filtered role
          department: dept,
          startDate,
          endDate,
          photoURL: "",
  ...
  return (
    <div className="flex min-h-screen bg-[#fafafa] relative">
      {/* Sidebar like in AdminDashboard */}
      <AdminSidebar
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      >
                        ...
                        {rolesList
                          .filter((r) => r.Name.toLowerCase() !== "admin") // 👈 Hide the "Admin" role
                          .map((r) => (
            ...
            <div className="flex justify-center border-b border-gray-200 bg-white">
              {["Resident Doctor", "Administration"].map((tab) => (
                <button
                  ...
                  onClick={() =>
                    setActiveRoleTab(
                      tab as "Resident Doctor" | "Administration"
                    )
                  }
