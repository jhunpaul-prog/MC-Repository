...
type User = {
  ...
  department?: string;
  status?: string;
  CreatedAt?: any; // may be number/string/Firebase timestamp
  updateDate?: string;
  lastName?: string;
  firstName?: string;
...
const ManageAccountAdmin = () => {
  ...
  // Roles: exclude any that contain "admin" or "super"
  const isAdminRole = (u: User) => {
    const role = (u.role || "").trim().toLowerCase();
    return role.includes("admin") || role.includes("super");
  };

  const nonAdminUsers = users.filter((u) => !isAdminRole(u));
  ...
  const handleStatusToggle = async (userId: string, currentStatus: string) => {
    ...
    setPendingStatus(currentStatus === "deactivate" ? "active" : "deactivate");
  };
  ...
  const computeStats = () => {
    ...
    const activeUsersCount = nonAdminUsers.filter(
      (u) => u.status !== "deactivate"
    ).length;
    const deactivatedUsersCount = nonAdminUsers.filter(
      (u) => u.status === "deactivate"
    ).length;
    ...
    const newUsersToday = nonAdminUsers.filter((u) => {
      ...
      if (!t) return false;
      return new Date(t).toISOString().slice(0, 10) === todayISO;
    }).length;
  ...
  return (
    <div className="flex min-h-screen bg-[#fafafa] relative">
      <AdminSidebar
        ...
        toggleSidebar={handleExpand} // fixed: expand
        notifyCollapsed={handleCollapse} // fixed: collapse
      />
