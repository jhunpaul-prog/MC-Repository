...
export async function deleteAccountHard(opts: {
  ...
  try {
    ...
    try {
      ...
      console.warn("Express server failed, falling back to client-side deletion:", serverError?.message);
      ...
      try {
        ...
        return {
          ...
          dbDeleted: true,
          authDeleted: false,
          error: "Database deleted successfully. Firebase Auth deletion requires server setup. Please delete manually from Firebase Console or start the Express server."
        };
      } catch (dbError: any) {
        return {
          ok: false,
          dbDeleted: false,
          authDeleted: false,
          error: `Database deletion failed: ${dbError?.message || String(dbError)}`
        };
      }
    }
  }
